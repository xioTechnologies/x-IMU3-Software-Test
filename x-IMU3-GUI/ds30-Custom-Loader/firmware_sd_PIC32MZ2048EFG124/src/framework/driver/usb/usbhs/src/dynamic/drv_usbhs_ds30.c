...
#include "driver/usb/usbhs/src/drv_usbhs_local_ds30.h"
        ...
        switch(hDriver->usbDrvCommonObj.state)
        {
            ...
            case DRV_USBHS_TASK_STATE_RUNNING:
                        ...
                        (DRV_USBHS_OPMODE_DEVICE == hDriver->usbDrvCommonObj.operationMode) )
                {
                    ...
                    if (hDriver->usbDrvCommonObj.vbusLevel != vbusLevel)
                    {
                        /* This means there was a change in the level */
                        if ((vbusLevel >= USBHS_VBUS_BELOW_VBUSVALID))
                        {
                            if (((vbusLevel == USBHS_VBUS_VALID) && (hDriver->usbDrvCommonObj.vbusLevel == USBHS_VBUS_BELOW_VBUSVALID))
                                    || ((vbusLevel == USBHS_VBUS_BELOW_VBUSVALID) && (hDriver->usbDrvCommonObj.vbusLevel == USBHS_VBUS_VALID)))
                            {
                                /* Do not do anything here. If the code flow
                                 * comes here, it means that VBUS transition
                                 * from USBHS_VBUS_BELOW_VBUSVALID to
                                 * USBHS_VBUS_VALID or vice versa. The callback
                                 * call and necessary data structures have
