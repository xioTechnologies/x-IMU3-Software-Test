...
)
{
    ...
    if( (DRV_HANDLE_INVALID !=  handle) && (NULL != ((DRV_USBHS_CLIENT_OBJ *)handle)) )
    {
        if(((DRV_USBHS_CLIENT_OBJ *)handle)->inUse)
        {
            ...
            if( (DRV_USBHS_OPMODE_DUAL_ROLE == hDriver->usbDrvCommonObj.operationMode)  && (true == hDriver->usbDrvCommonObj.isDeviceRoleActive) )
            {
                SYS_DEBUG_MESSAGE(SYS_ERROR_DEBUG, "\r\nUSBHS Driver: Inappropriate usage of Device attach in DRV_USBHS_DEVICE_Attach()");
            }
            else
            {
    ...
    if( (DRV_HANDLE_INVALID !=  handle) && (NULL != ((DRV_USBHS_CLIENT_OBJ *)handle)) )
    {
        if(((DRV_USBHS_CLIENT_OBJ *)handle)->inUse)
        {
            ...
            else
            {
                ...
                if(DRV_USBHS_OPMODE_DUAL_ROLE == hDriver->usbDrvCommonObj.operationMode)
                {
                    hDriver->usbDrvCommonObj.isDeviceRoleActive = false;
                    hDriver->usbDrvCommonObj.deviceModeClient = NULL;
                }
