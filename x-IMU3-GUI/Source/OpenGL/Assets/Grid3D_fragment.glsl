...
const float gridTilingFactor = 60.0;
// const vec3 graphBlue = vec3(0.196, 0.773, 1.0); // Unused for now
...
/** Outputs color for a grid to be drawn via UV texture coordinates.
    ...
    @param scale set the number of tiling repetitions
*/
vec4 grid(vec2 uvPos, float scale)
{
    ...
    float minDistanceFromOriginToEdge = 0.5;
    ...
    float normalizedDistanceFromOrigin = min(distanceFromOrigin, minDistanceFromOriginToEdge) / minDistanceFromOriginToEdge;
...
void main()
{
    fragColor = grid(textureCoordinate, gridTilingFactor);
} 
