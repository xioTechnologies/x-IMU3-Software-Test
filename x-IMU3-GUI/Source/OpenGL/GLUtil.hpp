...
namespace GLUtil
{
     ...
     */
    static juce::OpenGLShaderProgram::Uniform* createUniform(juce::OpenGLContext&, juce::OpenGLShaderProgram& shaderProgram, const juce::String& uniformName)
    {
        if (juce::gl::glGetUniformLocation(shaderProgram.getProgramID(), uniformName.toRawUTF8()) < 0)
        {
    ...
    /** Fail-safe method for creating an OpenGLShaderProgram::Attribute.
        ...
        this will notify you of the naming error and provides a descriptive error message.
     */
    static juce::OpenGLShaderProgram::Attribute* createAttribute(juce::OpenGLContext&, juce::OpenGLShaderProgram& shaderProgram, const juce::String& attributeName)
    {
        if (juce::gl::glGetAttribLocation(shaderProgram.getProgramID(), attributeName.toRawUTF8()) < 0)
        {
            /** If you hit this assertion, then you have attempted to create an
    ...
    {
    public:
        OpenGLNamedIDWrapper(const juce::String& name)
        {
            this->idName = name;
        }
