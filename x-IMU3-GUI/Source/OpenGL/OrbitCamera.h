...
// 3D camera that orbits a single 3D point
class OrbitCamera
{
public:
    ...
    glm::mat4 getProjectionMatrixOrthogonal() const
    {
        return projectionMatrixOrthogonal;
    }
...
private:
    ...
    {
        projectionMatrix = glm::perspective(glm::radians(fieldOfViewDegrees), aspectRatio, nearClip, farClip);

        const auto scale = 1.0f; // the orthogonal view of the camera could optionally be scaled (zoomed) with this value, but it is recommended to use a view matrix for this instead
        projectionMatrixOrthogonal = glm::ortho(-scale * aspectRatio, scale * aspectRatio, -scale, scale, nearClip, farClip); // ref: https://www3.ntu.edu.sg/home/ehchua/programming/opengl/CG_Examples.html
    }

    glm::mat4 projectionMatrix { 1.0f };
    glm::mat4 projectionMatrixOrthogonal { 1.0f };
    ...
    //glm::vec3 orbitOrigin { 0.0f, 0.0f, 0.0f }; // TODO: Actually implement this into setOrbit caluclation code
