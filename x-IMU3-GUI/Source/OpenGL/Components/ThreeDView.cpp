...
ThreeDView::ThreeDView(GLRenderer& renderer_) : OpenGLComponent(renderer_.getContext()),
                                                renderer(renderer_),
                                                worldGrid(renderer.getResources())
{
...
void ThreeDView::render()
{
    ...
    if (settings.isAxesEnabled)
    {
        ...
        // NOTE: This could be handled by GLRenderer, so text scale is always updated when viewport changes instead of manually updating here . . .
        text.setScale({ 1.0f / (float) bounds.getWidth(), 1.0f / (float) bounds.getHeight() }); // sets text scale to the normalized size of a screen pixel
...
void ThreeDView::renderWorldGrid(GLResources& resources, const glm::mat4& projectionMatrix, const glm::mat4& viewMatrix, const glm::mat4& axesConventionRotationGLM, const float floorHeight)
{
    ...
    // World Grid - tiles have width/height of 0.5 OpenGL units assuming `gridTilingFactor = 40.0` in Grid3D.frag
    ...
    worldGrid.modelViewProjectionMatrix = projectionMatrix * viewMatrix * translateGrid * scaleGrid * axesConventionRotationGLM;
    worldGrid.render();
...
// TODO: Need to make these axes a constant size (in pixels) too
void ThreeDView::renderAxesWorldSpace(GLResources& resources, const glm::mat4& deviceRotation, const glm::mat4& axesConventionRotation) const
{
    ...
    const auto projectionMatrix = camera.getProjectionMatrixOrthogonal();
...
void ThreeDView::renderAxesScreenSpace(GLResources& resources, const glm::mat4& axesConventionRotation) const
{
    ...
    const auto viewMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -1.0f)) * camera.getRotationMatrix();
    const auto projectionMatrix = camera.getProjectionMatrixOrthogonal();
    ...
    //unlitShader.colour.setRGBA(UIColours::graphRed);
    //unlitShader.modelViewProjectionMatrix.set(ndcMat * modelViewProjectionMatrix * xRotate);
    resources.arrow.render();
    ...
    //unlitShader.colour.setRGBA(UIColours::graphGreen);
    //unlitShader.modelViewProjectionMatrix.set(ndcMat * modelViewProjectionMatrix * yRotate);
    resources.arrow.render();
    ...
    //unlitShader.colour.setRGBA(UIColours::graphBlue);
    //unlitShader.modelViewProjectionMatrix.set(ndcMat * modelViewProjectionMatrix * zRotate);
    resources.arrow.render();
