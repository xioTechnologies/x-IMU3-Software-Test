...
// Macros
//------------------------------------------------------------------------------
#define sd_tx(d)	spi_exchange_byte( d )
#define sd_rx()		spi_exchange_byte( 0xFF )
...
//-----------------------------------------------------------------------
static BYTE select (void)	// 1:Successful, 0:Timeout 
{
	SD_CS = 0;      // Set CS# low 
...
//-----------------------------------------------------------------------
// NOTE: XC8 compiler is unable to allow recursion, so the send_cmd function had to be divided 
